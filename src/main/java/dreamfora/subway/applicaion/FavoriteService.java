...
import dreamfora.member.application.MemberService;
import dreamfora.member.application.dto.MemberResponse;
import dreamfora.subway.applicaion.dto.FavoriteRequest;
import dreamfora.subway.applicaion.dto.FavoriteResponse;
import dreamfora.subway.applicaion.dto.StationResponse;
import dreamfora.subway.domain.Favorite;
import dreamfora.subway.domain.FavoriteRepository;
...
public class FavoriteService {
    ...
    public List<FavoriteResponse> findFavorites(String email) {
        ...
        return favorites.stream().map(it -> FavoriteResponse.of(it, StationResponse.of(stations.get(it.getSourceStationId())), StationResponse.of(stations.get(it.getTargetStationId())))).collect(Collectors.toList());
    }
    ...
    private Map<Long, Station> extractStations(List<Favorite> favorites) {
        ...
        return stationService.findAllStationsById(stationIds).stream().collect(Collectors.toMap(Station::getId, Function.identity()));
    }
